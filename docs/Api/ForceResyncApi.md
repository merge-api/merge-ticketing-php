...
```php
syncStatusResyncCreate($xAccountToken): \OpenAPI\Client\Model\SyncStatus[]
```
...
$apiInstance = new OpenAPI\Client\Api\ForceResyncApi(
...
try {
    $result = $apiInstance->syncStatusResyncCreate($xAccountToken);
    print_r($result);
} catch (Exception $e) {
